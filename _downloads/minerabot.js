...
Markov.prototype = {
    ...
    {
        var newsize = tokens.length + 1;
        var newtokens = new Array(newsize);
    ...
    generate: function()
    {
        var maxlen = 20;
        var delimiter = " ";
        if (arguments.length >= 1) maxlen = parseInt(arguments[0]);
        if (arguments.length >= 2) delimiter = arguments[1];

        var len = 0;
        var key = this.startkey[Math.floor(Math.random() * this.startkey.length)];
        ...
        do
        {
            ...
            var target = keywords[Math.floor(Math.random() * keywords.length)];
