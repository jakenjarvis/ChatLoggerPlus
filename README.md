...
### DefaultReplaceLogFileFolderName デフォルトログフォルダ名（シンボル置換）
.minecraftフォルダからの相対パスを指定します。フォルダの区切り記号は「/」で区切ってください。
また、この項目は、後述の「シンボル置換処理」が入ります。

### DefaultReplaceLogFileName デフォルトログファイル名（シンボル置換）
保存するログファイル名を指定します。拡張子を含めて記述してください。
また、この項目は、後述の「シンボル置換処理」が入ります。

## absolutepath 絶対パス
### EnforcementReplaceLogFileFullPathFileName フルパスログファイル名（シンボル置換）
...
* %SERVERNAME%：サーバ名「（IPアドレスまたはドメイン名）:ポート番号」
* %PLAYERNAME%：プレイヤー名「ログインしているユーザー名」
* %DATE%：日付
* %TIME%：時間
